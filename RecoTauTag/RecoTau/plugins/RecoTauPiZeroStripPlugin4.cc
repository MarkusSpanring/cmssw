...
class RecoTauPiZeroStripPlugin4 : public RecoTauPiZeroBuilderPlugin 
{
 public:
  explicit RecoTauPiZeroStripPlugin4(const edm::ParameterSet&, edm::ConsumesCollector &&iC);
  virtual ~RecoTauPiZeroStripPlugin4();
 ...
 private:
  ...
  std::unique_ptr<const TFormula> seedEtaAssociationDistance_; // size of strip clustering window in eta direction
  std::unique_ptr<const TFormula> seedPhiAssociationDistance_; // size of strip clustering window in phi direction
  std::unique_ptr<const TFormula> candEtaAssociationDistance_; // size of strip clustering window in eta direction
  std::unique_ptr<const TFormula> candPhiAssociationDistance_; // size of strip clustering window in phi direction
...
RecoTauPiZeroStripPlugin4::RecoTauPiZeroStripPlugin4(const edm::ParameterSet& pset, edm::ConsumesCollector &&iC)
  : RecoTauPiZeroBuilderPlugin(pset, std::move(iC)),
    vertexAssociator_(pset.getParameter<edm::ParameterSet>("qualityCuts"), std::move(iC)), qcuts_(nullptr), seedEtaAssociationDistance_(nullptr), seedPhiAssociationDistance_(nullptr),candEtaAssociationDistance_(nullptr), candPhiAssociationDistance_(nullptr)
{
...
//-------------------------------------------------------------------------------
  ...
  edm::ParameterSet seedSize_eta_pset = pset.getParameterSet("seedEtaAssociationDistance");
  seedEtaAssociationDistance_ = makeFunction("seedEtaAssociationDistance", seedSize_eta_pset);
  edm::ParameterSet seedSize_phi_pset = pset.getParameterSet("seedPhiAssociationDistance");
  seedPhiAssociationDistance_ = makeFunction("seedPhiAssociationDistance", seedSize_phi_pset);

  edm::ParameterSet candSize_eta_pset = pset.getParameterSet("candEtaAssociationDistance");
  candEtaAssociationDistance_ = makeFunction("candEtaAssociationDistance", candSize_eta_pset);
  edm::ParameterSet candSize_phi_pset = pset.getParameterSet("candPhiAssociationDistance");
  candPhiAssociationDistance_ = makeFunction("candPhiAssociationDistance", candSize_phi_pset);
...
RecoTauPiZeroStripPlugin4::~RecoTauPiZeroStripPlugin4()
{
} 

// Update the primary vertex
void RecoTauPiZeroStripPlugin4::beginEvent() 
{
  vertexAssociator_.setEvent(*evt());
}

void RecoTauPiZeroStripPlugin4::addCandsToStrip(RecoTauPiZero& strip, CandPtrs& cands, const std::vector<bool>& candFlags, 
						std::set<size_t>& candIdsCurrentStrip, bool& isCandAdded) const
{
  if ( verbosity_ >= 1 ) {
    edm::LogPrint("RecoTauPiZeroStripPlugin4") << "<RecoTauPiZeroStripPlugin4::addCandsToStrip>:" ;
  }
  ...
  for ( size_t candId = 0; candId < numCands; ++candId ) {
    if ( (!candFlags[candId]) && candIdsCurrentStrip.find(candId) == candIdsCurrentStrip.end() ) { // do not include same cand twice
      ...
      double etaAssociationDistance_value = TMath::Min(etaAssociationDistance_, seedEtaAssociationDistance_->Eval(strip.pt()) + candEtaAssociationDistance_->Eval(cand->pt()) );
      double phiAssociationDistance_value = TMath::Min(phiAssociationDistance_, seedPhiAssociationDistance_->Eval(strip.pt()) + candPhiAssociationDistance_->Eval(cand->pt()) );
	...
	if ( verbosity_ >= 2 ) {
	  edm::LogPrint("RecoTauPiZeroStripPlugin4") << "--> adding PFCand #" << candId << " (" << cand.id() << ":" << cand.key() << "): Et = " << cand->et() << ", eta = " << cand->eta() << ", phi = " << cand->phi() ;
	}
...
RecoTauPiZeroStripPlugin4::return_type RecoTauPiZeroStripPlugin4::operator()(const reco::Jet& jet) const 
{
  if ( verbosity_ >= 1 ) {
    edm::LogPrint("RecoTauPiZeroStripPlugin4") << "<RecoTauPiZeroStripPlugin4::operator()>:" ;
    edm::LogPrint("RecoTauPiZeroStripPlugin4") << " minGammaEtStripSeed = " << minGammaEtStripSeed_ ;
    edm::LogPrint("RecoTauPiZeroStripPlugin4") << " minGammaEtStripAdd = " << minGammaEtStripAdd_ ;
    edm::LogPrint("RecoTauPiZeroStripPlugin4") << " minStripEt = " << minStripEt_ ;
  }
  ...
  for ( CandPtrs::iterator cand = candsVector.begin();
    ...
    if ( verbosity_ >= 1 ) {
      edm::LogPrint("RecoTauPiZeroStripPlugin4") << "PFGamma #" << idx << " (" << cand->id() << ":" << cand->key() << "): Et = " << (*cand)->et() << ", eta = " << (*cand)->eta() << ", phi = " << (*cand)->phi() ;
    } 
    if ( (*cand)->et() > minGammaEtStripSeed_ ) {
      if ( verbosity_ >= 2 ) {
	edm::LogPrint("RecoTauPiZeroStripPlugin4") << "--> assigning seedCandId = " << seedCands.size() ;
        const reco::Track* candTrack = getTrack(**cand);
        if ( candTrack ) {
	  edm::LogPrint("RecoTauPiZeroStripPlugin4") << "track: Pt = " << candTrack->pt() << " eta = " << candTrack->eta() << ", phi = " << candTrack->phi() << ", charge = " << candTrack->charge() ;
	  edm::LogPrint("RecoTauPiZeroStripPlugin4") << " (dZ = " << candTrack->dz(vertexAssociator_.associatedVertex(jet)->position()) << ", dXY = " << candTrack->dxy(vertexAssociator_.associatedVertex(jet)->position()) << "," 
	...
	// edm::LogPrint("RecoTauPiZeroStripPlugin4") << "ECAL Et: calibrated = " << (*cand)->ecalEnergy()*sin((*cand)->theta()) << "," 
		  // << " raw = " << (*cand)->rawEcalEnergy()*sin((*cand)->theta()) ;
	// edm::LogPrint("RecoTauPiZeroStripPlugin4") << "HCAL Et: calibrated = " << (*cand)->hcalEnergy()*sin((*cand)->theta()) << "," 
		  // << " raw = " << (*cand)->rawHcalEnergy()*sin((*cand)->theta()) ;
      }
      seedCands.push_back(*cand);
    } else if ( (*cand)->et() > minGammaEtStripAdd_  ) {
      if ( verbosity_ >= 2 ) {
	edm::LogPrint("RecoTauPiZeroStripPlugin4") << "--> assigning addCandId = " << addCands.size() ;
      }
  ...
  while ( idxSeed < seedCands.size() ) {
    if ( verbosity_ >= 2 ) edm::LogPrint("RecoTauPiZeroStripPlugin4") << "processing seed #" << idxSeed ;
    ...
    do {
      ...
      //if ( verbosity_ >= 2 ) edm::LogPrint("RecoTauPiZeroStripPlugin4") << " adding seedCands to strip..." ;
      addCandsToStrip(*strip, seedCands, seedCandFlags, seedCandIdsCurrentStrip, isCandAdded);
      //if ( verbosity_ >= 2 ) edm::LogPrint("RecoTauPiZeroStripPlugin4") << " adding addCands to strip..." ;
    ...
    if ( strip->et() > minStripEt_ ) { // strip passed Et cuts, add it to the event
      if ( verbosity_ >= 2 ) edm::LogPrint("RecoTauPiZeroStripPlugin4") << "Building strip: Et = " << strip->et() << ", eta = " << strip->eta() << ", phi = " << strip->phi() ;
      ...
      if ( verbosity_ >= 2 ) edm::LogPrint("RecoTauPiZeroStripPlugin4") << "Discarding strip: Et = " << strip->et() << ", eta = " << strip->eta() << ", phi = " << strip->phi() ;
    }
...
DEFINE_EDM_PLUGIN(RecoTauPiZeroBuilderPluginFactory,
    reco::tau::RecoTauPiZeroStripPlugin4, "RecoTauPiZeroStripPlugin4");
